// Excel運用HTML化ツール ①②のコア処理（ブラウザ・Node両方で動く共通ロジック）
...
function buildRowEntry(grid, u0, u1, maxCol, fromCol, getValue, collector, context) {
  getValue = getValue || defaultGetValue;
  // このユニット（u0〜u1）の範囲を超えて伸びる結合セル（row2 > u1）は、この行1行だけの
  // 見出しにはなり得ない（元々グループ全体の見出しだったもの。手動グループ化の解除・
  // 再グループ化等を経てアンカー行だけがここに単独で流れ込んでくることがある）。
  // 「行の先頭セル＝この行のラベル」「行途中の見出し＝直後の値の項目名」のどちらの判定
  // からも除外し、この行には存在しないものとして扱う（残す場合、アンカー行だけラベル
  // 扱いになり配列から漏れたり、直後のセルの項目名として誤って使われたりする）。
  // 判定基準はrow2 > c.row（2行以上にまたがるか）ではなくrow2 > u1（このユニット自体の
  // 範囲を超えて伸びているか）とする。u1はrowSpanOf（1列目を除く2列目以降の結合状況）で
  // 決まるため、ユニット内で完結する2行結合（例：見た目の行高調整のためだけに2行結合
  // されている「未達成の理由」等の通常のラベル）はここでは除外されない。ユニットの範囲を
  // 超えて複数ユニットにまたがる本物のグループアンカー（例：A11:A19）だけが除外される。
  const cells = cellsOfRow(grid, u0, maxCol, fromCol)
    .filter(c => !c.isFormula)
    .filter(c => !(c.hasText && c.row2 > u1));
